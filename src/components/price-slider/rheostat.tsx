...
const Rheostat = (props)=> {
  ...
  const [values, setValues] = React.useState([40, 60])

  const updateValue = (sliderState) => {
    setValues(sliderState.values)
  }
  const MyHandle= ({
                      styles,
                      css,
  ...
  const { formatValue, ...passProps } = props;
  ...
  return (
    ...
    >
      <Rheostat
        ...
        algorithm={log10}
        snap
        snapPoints={[20, 40, 50, 58,  70]}
        {...passProps}
        onValuesUpdated={updateValue}
        values={values}
        handle={StyledMyHandle}
      />
      <ol>
        <lh>Values</lh>
        {values.map((value) => (
